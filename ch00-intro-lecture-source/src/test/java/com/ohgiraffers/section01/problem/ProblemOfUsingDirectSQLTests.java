...
public class ProblemOfUsingDirectSQLTests {
    ...
    * 1. 데이터 반환, SQL 작성, JDBC API 코드 등의 중복 작성
    * 2. SQL에 의존하여 개발
    * 3. 패러다임 불일치 (상속, 연관관계, 객체 그래프 탐색)
    * 4. 동일성 보장문제
    * */

    // 1. 데이터 반환, SQL 작성 등의 중복 작성 문제
...
//        menuList.forEach(menu -> System.out.println(menu)); // menuList 전체 출력
        menuList.forEach(System.out::println);  // menuList 전체 출력
...
//        repository.save(menu);
    ...
    * 데이터베이스
    * Menu menu = new Menu();
    * Category category = new Catogory();
    * menu.setCategory(catetory);
    ...
    * */
     ...
     * menu.getCatetory().getCategoryName();    // NPE 발생X
     * */
